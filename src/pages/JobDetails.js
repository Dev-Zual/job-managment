...
const JobDetails = () => {
  ...
  const { data, isLoading2 } = useGetJobByIdQuery(id);
  const [askQuestions, { isLoading3, isSuccess2 }] = useAskQuestionsMutation();
  ...
  useEffect(() => {
    ...
    if (!isLoading && isSuccess) {
      toast.success("successfully applied..", { id: "applying" });
    }
    if (isLoading3) {
      toast.loading("your question is posting");
    }
    if (!isLoading3 && isSuccess2) {
      toast.loading("your question is posted");
    }
  });
  const {
    companyName,
        ...
        <div className="space-y-5">
          <div className="flex justify-between items-center mt-5">
            ...
            <div>
              ...
              {status === "active" ? (
                <span className=" px-2 text-green-500 py-1 ">Active</span>
              ) : (
                <span className="px-2 py-1 text-red-500 ">Closed</span>
              )}
            </div>
